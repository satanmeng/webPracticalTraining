...
class NodeAction extends CommonAction {
    ...
    public function _filter(&$map) {
        ...
        $_REQUEST['_order'] = empty($_REQUEST['_order']) ? 'group_id' : $_REQUEST['_order'];
        $_REQUEST['_sort'] = empty($_REQUEST['_sort']) ? 'asc,sort asc' : $_REQUEST['_sort'];
    }

    public function _before_index() {
        $model = M("Group");
        $list = $model->where('status=1')->field('id,title')->select();
