...
class Wechat
{
	...
	const CARD_LIST_GET                   = '/card/user/getcardlist?';     //获取用户已领取卡券接口
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	private function http_get($url){
        log::write("get: " . $url, LOG::INFO, 3, '/var/www/webroot/nzg/logs/wap/wechat.log');
        log::write($this->access_token, LOG::INFO, 3, '/var/www/webroot/nzg/logs/wap/wechat.log');
		$oCurl = curl_init();
        ...
        log::write(print_r($sContent, true), LOG::INFO, 3, '/var/www/webroot/nzg/logs/wap/wechat.log');
		curl_close($oCurl);
	...
	private function http_post($url,$param,$post_file=false){
        log::write("post: " . $url, LOG::INFO, 3, '/var/www/webroot/nzg/logs/wap/wechat.log');
        log::write($this->access_token, LOG::INFO, 3, '/var/www/webroot/nzg/logs/wap/wechat.log');
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
	...
	public function createMenu($data){
		...
		if ($result) {
			$json = json_decode($result,true);
			log::write(print_r($json,true),LOG::INFO);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
				return false;
	...
	public function sendMassMessage($data){
		...
		{
			$json = json_decode($result,true);
            log::write(print_r($result, true), LOG::INFO);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
				return false;
...
//        log::write(print_r($callback, true), LOG::INFO);
//        log::write(print_r($state, true), LOG::INFO);
//        log::write(print_r($scope, true), LOG::INFO);
    ...
    public function delCard($card_id) {
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        log::write(print_r($data, TRUE), LOG::INFO);
        log::write(print_r($this->access_token, TRUE), LOG::INFO);
     ...
     */
    public function getUserCardlist($openid) {
        $data = array(
            'openid' => $openid,
            'card_id' => ''
        );
        if (!$this->access_token && !$this->checkAuth()) return false;       
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_LIST_GET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array   返回数组请参看 微信卡券接口文档 的json格式
     */
    public function getPoiList($offset = 0, $count = 20) {
        $data = array(
            'begin' => $offset,
            'limit' => $count
        );
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::POI_LIST . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
